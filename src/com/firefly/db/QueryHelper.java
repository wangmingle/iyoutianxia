...
public class QueryHelper {
	 ...
	 * @throws SQLException 数据库出错时抛出异常
	 */
	public static <T> T read(Class<T> beanClass, String sql) throws SQLException {
	    // 判断是否为原始类型，如果是就用_scaleHandler，否则根据beanClass参数生成一个BeanHandler
        return (T) _runner.query(getConnection(), sql,
                _IsPrimitive(beanClass) ? _scaleHandler
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static <T> List<T> query(Class<T> beanClass, String sql) throws SQLException {
		_runner.update(getConnection(), "set @x =0");
	    return (List<T>) _runner.query(getConnection(), sql,_IsPrimitive(beanClass) ? _columnListHandler: new BeanListHandler(beanClass));
	}
	...
	public static int update(String sql) throws SQLException {
		return _runner.update(getConnection(), sql);
	}
